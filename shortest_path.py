...
FULL_PATH = "/Users/nedasoltani/Workspaces/Environments/SNA/my_env/"
PREPROCESS = False
...
def connect(r_type, records, id_label, distance_function, similarity_threshold=0):
        ...
        for j in range(i+1, len(records)):
            if i != j:
                ...
                dist = distance_functions.calculate_distance(
                    records.ix[i][id_indx:], records.ix[j][id_indx:], distance_function, PREPROCESS, id_label)
                if dist == 0:
                    sim = 2
                else:
                    if dist == 1:
...
def main():
    ...
    for u_sim_threshold in numpy.arange(0.7, 1.0, 0.2):
        for s_sim_threshold in numpy.arange(0.7, 1.0, 0.2):
            for distance_function in ["euclidean"]:#, "sorensen", "cosine", "jaccard", "hamming"]:
                # connect and create social network
                connected_users = connect(
                    True, preprocessed_users, config.u_id_label, distance_function, u_sim_threshold)
                connected_services = connect(
                    False, preprocessed_services, config.s_id_label, distance_function, s_sim_threshold)
                ...
                for shortest_path_algorithm in ["dijkstra", "bellman_ford"]:
